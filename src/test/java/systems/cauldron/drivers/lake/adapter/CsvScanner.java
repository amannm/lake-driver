...
public class CsvScanner {
    ...
    private List<Long> pageStartOffsets = new ArrayList<>();
    ...
    public void writePage(int pageIndex) throws IOException {

        System.out.println("writing page: " + pageIndex);
        ...
        if (buffer.hasRemaining()) {
            Stream.of(StandardCharsets.UTF_8.decode(buffer).toString().split("\\n"))
                    .map(row -> row.split(","))
                    .forEach(cache::add);
        }
        buffer.clear();
